...
class OpenDriftSimulation(PhysicsMethods, Timeable):
	...
	def seed_elements(self, lon, lat, time, radius=0, number=None,
		...
		if 'z' in kwargs and isinstance(kwargs['z'], str) \
			...
			if len(kwargs['z']) > 8 and kwargs['z'][8] == '+':
				meters_above_seafloor = np.float_(kwargs['z'][9::])
	...
	def seed_cone(self, lon, lat, time, radius=0, number=None, **kwargs):
		...
		radius = radius.astype(np.float_)
		...
		geo = geojson.LineString([(np.float_(lonin[0]), np.float_(latin[0])),
								  (np.float_(lonin[1]), np.float_(latin[1]))])
	...
	def seed_repeated_segment(self, lons, lats,
		...
		if number_per_segment is None:
			number_per_segment = np.int_(np.floor(total_number/numtimes))
	...
	def seed_from_wkt(self, wkt, number=None, **kwargs):
		...
		for i in range(0, geom.GetGeometryCount()):
			g = geom.GetGeometryRef(i)
			num_elements = np.int_(number*g.GetArea()/total_area)
	...
	def seed_from_shapefile(self, shapefile, number,
		...
		for layer in s:
			...
			numbers[numbers.argmax()] += np.int_(number-sum(numbers))
	...
	def set_up_map(self, corners=None, buffer=.1, delta_lat=None,
		...
		aspect_ratio = np.float_(latmax-latmin) / (np.float_(lonmax-lonmin))
	...
	def write_netcdf_density_map(self, filename, pixelsize_m='auto'):
		...
		if pixelsize_m == 'auto':
			...
			logger.info('\tPIXEL SIZE TO DENSITY MAP IS %s m' % pixelsize_m)
	...
	def plot_property(self, prop, filename=None, mean=False):
		...
		plt.plot(times, data)
		plt.title(prop)
		plt.xlabel('Time  [UTC]',size = 'small')
		...
		try:
			plt.ylabel('%s  [%s]' %
					   (prop, self.elements.variables[prop]['units']),size = 'x-small')
		except:
			plt.ylabel(prop,size = 'x-small')
		plt.subplots_adjust(bottom=.3)
		plt.grid('on', which='major', axis='both',color='k', alpha=0.2)
		if filename is None:
			plt.show()
		else:
			plt.savefig(filename)
