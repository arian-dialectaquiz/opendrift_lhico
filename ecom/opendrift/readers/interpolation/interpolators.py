...
class Nearest2DInterpolator():
    ...
    def __init__(self, xgrid, ygrid, x, y):
        ...
        self.xi = np.round(self.xi).astype(int)
        self.yi = np.round(self.yi).astype(int)
...
class Nearest1DInterpolator():
    ...
    def __init__(self, zgrid, z):
        ...
        self.zi = np.round(z_interpolator(z)).astype(int)
        self.zi[self.zi < 0] = 0
...
class Linear1DInterpolator():
    ...
    def __init__(self, zgrid, z):
        ...
        self.index_above = np.floor(interp_zi).astype(int)
