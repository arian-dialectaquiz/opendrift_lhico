...
class LarvalFish(OceanDrift):
	...
	def update_fish_larvae(self):
		...
		if len(eggs) > 0:
			...
			amb_duration = np.exp(3.65 - 0.12*self.environment.sea_water_temperature[eggs]) # Total egg development time (days) according to ambient temperature (Ellertsen et al. 1988)
		...
		self.elements.length[larvae] = (np.exp(2.296 + 0.277 * np.log(w) - 0.005128 *np.log10(w)**2))/12
	...
	def larvae_vertical_migration(self):
		...
		self.elements.z[larvae] = (np.minimum(0, self.elements.z[larvae] + direction*max_migration_per_timestep))/10
